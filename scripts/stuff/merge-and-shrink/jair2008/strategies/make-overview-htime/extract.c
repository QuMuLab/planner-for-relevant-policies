...

{
  ...
  for ( i = 0; i < 8; i++ ) {
    ...
    while ( TRUE ) {
      if ( (fscanf(fdata, "%d %d %d %f %d %d %f %f\n",
	     &tempN, &l, &h, &(htime[i][j]), &Slb, &S, &Stime, &(time[i][j]))) == EOF )
	break;
      if ( i == 0 ) {
	N[j] = tempN;
	num_problems++;
  ...
  if ( 0 ) {
	...
	printf("\n%s:\n", domain[domaincount++]);
      ...
      for ( i = 0; i < 8; i++ ) {
	if ( !gon[i] ) continue;
	printf("%s: %7.2f   |   ", strategy[i], time[i][j]);
      }
      printf("\n");
    }
  }
  ...
  for ( j = 0; j < num_problems; j++ ) {
    ...
    for ( i = 0; i < 8; i++ ) {
      ...
      thtime[i][current_domain][current_instance] =  htime[i][j];
      if ( time[i][j] != -1 ) {
  ...
  if ( 0 ) {
	...
	for ( i = 0; i < 8; i++ ) {
	  if ( !gon[i] ) continue;
	  printf("%s: %7.2f %d   |   ", strategy[i], ttime[i][k][j], solved[i][k][j]);
	}
	printf("\n");
      }
    }
  ...
  for ( k = 0; k < domaincount; k++ ) {
    for ( i = 0; i < 8; i++ ) {
      ...
      for ( j = 0; j < numinstances[k]; j++ ) {
	if ( allsolved[k][j] ) {
	  allsolvedmean[i][k] += thtime[i][k][j];
	  numallsolve++;
	}
      }
