top(col2row7,col2row6).
top(col2row6,col2row5).
top(col2row5,col2row4).
top(col2row4,col2row3).
top(col2row3,col2row2).
right(col2row2,col3row2).
top(col3row2,col3row1).
right(col3row1,col4row1).
top(col4row2,col4row1).
right(col4row2,col5row2).
top(col5row2,col5row1).
top(col5row3,col5row2).
top(col5row4,col5row3).
right(col5row4,col6row4).
top(col6row4,col6row3).
top(col6row3,col6row2).
right(col6row3,col7row3).
top(col7row4,col7row3).
top(col7row5,col7row4).
top(col6row5,col6row4).
top(col6row6,col6row5).
top(col6row7,col6row6).
top(col5row7,col5row6).
right(col5row6,col6row6).
top(col4row7,col4row6).
right(col4row7,col5row7).
top(col3row7,col3row6).
right(col3row6,col4row6).
right(col3row7,col4row7).
right(col4row6,col5row6).
right(col5row7,col6row7).
right(col6row5,col7row5).
right(col6row4,col7row4).
right(col5row3,col6row3).
right(col5row2,col6row2).
right(col4row1,col5row1).
top(col3row3,col3row2).
top(col3row4,col3row3).
right(col3row2,col4row2).
right(col2row3,col3row3).
right(col2row4,col3row4).
top(col1row5,col1row4).
top(col1row6,col1row5).
right(col1row6,col2row6).
right(col1row5,col2row5).
right(col1row4,col2row4).
right(col2row6,col3row6).
right(col2row7,col3row7).
box(col2row5).
box(col2row3).
box(col4row2).
box(col6row3).
box(col6row5).
box(col5row6).
box(col3row6).
solution(col5row2).
solution(col5row3).
solution(col5row4).
solution(col6row4).
solution(col6row2).
solution(col7row3).
solution(col7row4).
sokoban(col2row7).
step(1).
step(2).
step(3).
step(4).
step(5).
step(6).
step(7).
step(8).
step(9).
step(10).
step(11).
step(12).
step(13).
step(14).
step(15).
step(16).
step(17).
step(18).
step(19).
step(20).
step(21).
step(22).
step(23).
step(24).
next(1,2).
next(2,3).
next(3,4).
next(4,5).
next(5,6).
next(6,7).
next(7,8).
next(8,9).
next(9,10).
next(10,11).
next(11,12).
next(12,13).
next(13,14).
next(14,15).
next(15,16).
next(16,17).
next(17,18).
next(18,19).
next(19,20).
next(20,21).
next(21,22).
next(22,23).
next(23,24).
