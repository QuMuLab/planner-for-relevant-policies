...
def parse_prp(outfile):
    ...
    successful_states = get_value(outfile, '.*Successful states: (\d+)\n.*', int)
    replans = get_value(outfile, '.*Replans: (\d+)\n.*', int)
    actions = get_value(outfile, '.*Actions: (\d+)\n.*', int)
    size = get_value(outfile, '.*State-Action Pairs: (\d+)\n.*', int)
    
    strongly_cyclic = match_value(outfile, '.*Strongly Cyclic: True.*')
    succeeded = match_value(outfile, '.*Succeeded: True.*')
