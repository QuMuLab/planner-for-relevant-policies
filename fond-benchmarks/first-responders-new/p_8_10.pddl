(define (problem FR_8_10)
 (:domain first-response)
 (:objects  l1 l2 l3 l4 l5 l6 l7 l8  - location
	    f1 f2 f3 f4 f5 f6 - fire_unit
	    v1 v2 v3 v4 v5 v6 v7 v8 v9 v10 - victim
	    m1 m2 m3 m4 m5 m6 - medical_unit
)
 (:init 
	;;strategic locations
     (hospital l5)
     (hospital l7)
     (hospital l7)
     (hospital l1)
     (hospital l3)
     (hospital l1)
     (hospital l7)
     (water-at l4)
     (water-at l5)
     (water-at l3)
     (water-at l7)
     (water-at l2)
	;;disaster info
     (fire l7)
     (victim-at v1 l7)
     (victim-status v1 dying)
     (fire l1)
     (victim-at v2 l1)
     (victim-status v2 hurt)
     (fire l8)
     (victim-at v3 l6)
     (victim-status v3 hurt)
     (fire l5)
     (victim-at v4 l1)
     (victim-status v4 hurt)
     (fire l8)
     (victim-at v5 l8)
     (victim-status v5 dying)
     (fire l7)
     (victim-at v6 l5)
     (victim-status v6 dying)
     (fire l8)
     (victim-at v7 l8)
     (victim-status v7 hurt)
     (fire l6)
     (victim-at v8 l7)
     (victim-status v8 dying)
     (fire l3)
     (victim-at v9 l2)
     (victim-status v9 dying)
     (fire l1)
     (victim-at v10 l5)
     (victim-status v10 dying)
	;;map info
	(adjacent l1 l1)
	(adjacent l2 l2)
	(adjacent l3 l3)
	(adjacent l4 l4)
	(adjacent l5 l5)
	(adjacent l6 l6)
	(adjacent l7 l7)
	(adjacent l8 l8)
   (adjacent l1 l1)
   (adjacent l1 l1)
   (adjacent l2 l1)
   (adjacent l1 l2)
   (adjacent l2 l2)
   (adjacent l2 l2)
   (adjacent l2 l3)
   (adjacent l3 l2)
   (adjacent l3 l1)
   (adjacent l1 l3)
   (adjacent l4 l1)
   (adjacent l1 l4)
   (adjacent l4 l2)
   (adjacent l2 l4)
   (adjacent l4 l3)
   (adjacent l3 l4)
   (adjacent l4 l4)
   (adjacent l4 l4)
   (adjacent l4 l5)
   (adjacent l5 l4)
   (adjacent l4 l6)
   (adjacent l6 l4)
   (adjacent l5 l1)
   (adjacent l1 l5)
   (adjacent l7 l1)
   (adjacent l1 l7)
   (adjacent l7 l2)
   (adjacent l2 l7)
   (adjacent l7 l3)
   (adjacent l3 l7)
   (adjacent l7 l4)
   (adjacent l4 l7)
   (adjacent l8 l1)
   (adjacent l1 l8)
   (adjacent l8 l2)
   (adjacent l2 l8)
   (adjacent l8 l3)
   (adjacent l3 l8)
   (adjacent l8 l4)
   (adjacent l4 l8)
   (adjacent l8 l5)
   (adjacent l5 l8)
	(fire-unit-at f1 l3)
	(fire-unit-at f2 l4)
	(fire-unit-at f3 l4)
	(fire-unit-at f4 l7)
	(fire-unit-at f5 l7)
	(fire-unit-at f6 l6)
	(medical-unit-at m1 l1)
	(medical-unit-at m2 l8)
	(medical-unit-at m3 l8)
	(medical-unit-at m4 l8)
	(medical-unit-at m5 l5)
	(medical-unit-at m6 l5)
	)
 (:goal (and  (nfire l7) (nfire l1) (nfire l8) (nfire l5) (nfire l8) (nfire l7) (nfire l8) (nfire l6) (nfire l3) (nfire l1)  (victim-status v1 healthy) (victim-status v2 healthy) (victim-status v3 healthy) (victim-status v4 healthy) (victim-status v5 healthy) (victim-status v6 healthy) (victim-status v7 healthy) (victim-status v8 healthy) (victim-status v9 healthy) (victim-status v10 healthy)))
 )
