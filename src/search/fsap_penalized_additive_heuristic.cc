#include "fsap_penalized_additive_heuristic.h"
...
FSAPPenalizedAdditiveHeuristic::FSAPPenalizedAdditiveHeuristic(const Options &opts)
    : RelaxationHeuristic(opts),
      did_write_overflow_warning(false) {
    g_heuristic_for_reachability = this;
}

FSAPPenalizedAdditiveHeuristic::~FSAPPenalizedAdditiveHeuristic() {
}

void FSAPPenalizedAdditiveHeuristic::write_overflow_warning() {
...
// initialization
void FSAPPenalizedAdditiveHeuristic::initialize() {
...
void FSAPPenalizedAdditiveHeuristic::setup_exploration_queue() {
    queue.clear();
...
void FSAPPenalizedAdditiveHeuristic::setup_exploration_queue_state(const StateInterface &state) {
    for (int var = 0; var < propositions.size(); var++) {
        if (-1 == state[var]) {
...
bool FSAPPenalizedAdditiveHeuristic::relaxed_exploration(bool include_forbidden) {
...
void FSAPPenalizedAdditiveHeuristic::mark_preferred_operators(
...
int FSAPPenalizedAdditiveHeuristic::compute_add_and_ff(const StateInterface &state) {
...
int FSAPPenalizedAdditiveHeuristic::compute_heuristic(const State &state) {
...
static Heuristic *_parse(OptionParser &parser) {
    parser.document_synopsis("FSAP Penalized Additive heuristic", "");
    ...
    if (parser.dry_run())
        ...
        return new FSAPPenalizedAdditiveHeuristic(opts);
}

static Plugin<Heuristic> _plugin("fsappenalizeadd", _parse);
