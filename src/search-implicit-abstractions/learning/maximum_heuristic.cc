...
//		arff_out << "@ATTRIBUTE f NUMERIC" << endl;
		...
		//arff_out << "@ATTRIBUTE dist NUMERIC" << endl;
		//arff_out << "@ATTRIBUTE h1 NUMERIC" << endl;
		//arff_out << "@ATTRIBUTE h2 NUMERIC" << endl;
		//arff_out << "@ATTRIBUTE hdiff NUMERIC" << endl;
		//arff_out << "@ATTRIBUTE hratio NUMERIC" << endl;
...
//		arff_out << g << ", " << max + g << endl;
		...
		int dist = 0;
		for (int i = 0; i < g_variable_domain.size(); i++) {
			if (state[i] != (*g_initial_state)[i])
				dist++;
		}


		arff_out << dist*1.0/g_variable_domain.size() << ", " << hvalue[0]<< ", " << hvalue[1] << ", "
		         << hvalue[0] - hvalue[1]<< ", " << hvalue[0] *1.0 / hvalue[1]<<  endl;
		         */
