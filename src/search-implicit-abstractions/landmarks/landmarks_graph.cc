...
void LandmarksGraph::dump_dot() const {
	ofstream dot_out("/tmp/landmarkgraph.dot");
    dot_out << "digraph graphname { " << endl;
    ...
    for (set<LandmarkNode*>::const_iterator it = nodes.begin(); it
        ...
        for (hash_map<LandmarkNode*, edge_type, hash_pointer>::const_iterator
    		...
    		dot_out << "\"";
            for (unsigned int i = 0; i < parent_p->vars.size(); i++) {
            	if (i>0)
            		dot_out << "_";

            	dot_out << parent_p->vars[i] << ":"<< parent_p->vals[i];
            }
            dot_out << "\" -> \"";
            ...
            for (unsigned int i = 0; i < node_p->vars.size(); i++) {
            	...
            	dot_out << node_p->vars[i] << ":"<< node_p->vals[i];
            }
