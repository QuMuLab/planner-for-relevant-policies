...
bool SearchNode::need_eval() const {
    return (info.need_eval == 1);
}

void SearchNode::mark_to_eval() {
    info.need_eval = 1;
}

void SearchNode::mark_not_eval() {
    info.need_eval = 0;
}
...
__gnu_cxx::hash_map<StateProxy, SearchNodeInfo>::const_iterator
SearchSpace::begin() const {
    return nodes->begin();
}

__gnu_cxx::hash_map<StateProxy, SearchNodeInfo>::const_iterator
SearchSpace::end() const {
    return nodes->end();
}
